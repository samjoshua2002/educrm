...
export default function OrganizationFormBuilderPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (isError || !form) {
    return (
      <div className="flex flex-col items-center justify-center h-screen space-y-4">
        ...
        <h2 className="text-xl font-semibold">Form not found or failed to load</h2>
    ...
    <div className="flex flex-col h-[calc(100vh-3rem)] overflow-hidden bg-muted/20">
      <Tabs defaultValue="build" className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
              <div className="flex items-center gap-2">
                ...
                <Badge variant={localStatus === "active" ? "default" : "secondary"} className="text-[10px] h-4 uppercase translate-y-[1px]">
                  {localStatus}
                </Badge>
              </div>
              <span className="text-[10px] text-muted-foreground uppercase tracking-widest font-bold">
                {form.updatedAt ? `Last updated ${format(new Date(form.updatedAt), 'hh:mm a')}` : 'Not saved'}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button size="sm" className="h-8 font-bold bg-primary shadow-lg shadow-primary/20" disabled={isSaving}>
              ...
              <DropdownMenuContent align="end" className="w-48">
                <DropdownMenuItem className="font-semibold cursor-pointer" onClick={() => handleSave(false)}>
                  <Save className="h-4 w-4 mr-2" /> Save as Draft
                </DropdownMenuItem>
                <DropdownMenuSeparator />
                <DropdownMenuItem className="font-semibold cursor-pointer text-primary focus:text-primary" onClick={() => handleSave(true)}>
        ...
        <main className="flex-1 flex overflow-hidden">
          <TabsContent value="build" className="flex-1 flex m-0 overflow-hidden">
            {/* Left Sidebar: Components */}
            <aside className="w-72 border-r bg-background flex flex-col shrink-0">
              <ScrollArea className="flex-1">
                <div className="p-4 border-b">
                  <h2 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-4">Form Elements</h2>
            ...
            <div className="flex-1 bg-muted/20 overflow-y-auto p-8 flex justify-center">
              <Card className="w-full max-w-2xl min-h-[800px] h-fit p-10 shadow-xl border-none ring-1 ring-slate-200">
                <div className="space-y-4">
                  ...
                  {localFields.length === 0 && (
                    <div className="py-20 text-center border-2 border-dashed rounded-2xl flex flex-col items-center justify-center gap-4 bg-slate-50/50">
                      ...
                      <div>
                        <p className="font-bold text-slate-400 uppercase tracking-widest text-[10px]">Canvas Empty</p>
                        <p className="text-sm text-slate-400 max-w-[200px] mt-1">Start by adding fields from the left sidebar.</p>
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="flex justify-end pt-4">
                  <Button disabled className="px-10 h-11 font-bold text-sm uppercase tracking-wide">Submit Application</Button>
                </div>
              </Card>
            </div>
            ...
            <aside className="w-80 border-l bg-background shrink-0 flex flex-col overflow-hidden">
              {selectedField ? (
                <ScrollArea className="flex-1">
                  <div className="p-6 space-y-8">
                    ...
                    <div>
                      ...
                      <div className="space-y-5">
                        <div className="space-y-2">
                          <Label htmlFor="field-label" className="text-[10px] font-bold uppercase text-muted-foreground tracking-widest">Display Label</Label>
                          <Input 
                            id="field-label"
                        ...
                        <div className="space-y-2">
                          <Label htmlFor="field-placeholder" className="text-[10px] font-bold uppercase text-muted-foreground tracking-widest">Placeholder Text</Label>
                          <Input 
                    ...
                    <div className="space-y-6">
                      <h4 className="text-[10px] font-bold uppercase text-muted-foreground tracking-widest">Behavior & Logic</h4>
                      ...
                      <div className="flex items-center justify-between p-3 rounded-lg bg-slate-50 ring-1 ring-slate-200">
                        <div className="space-y-0.5">
                          ...
                          <p className="text-[10px] text-muted-foreground">Force applicant to answer</p>
                        </div>
                        <Switch 
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-10 text-center space-y-4 opacity-40 grayscale">
                  ...
                  <div>
                    <p className="font-bold text-[10px] uppercase tracking-widest">No Field Selected</p>
                    <p className="text-xs text-muted-foreground mt-1 max-w-[140px]">Select a field in the canvas to configure its settings.</p>
                  </div>
                </div>
              )}
            </aside>
          ...
          <TabsContent value="settings" className="flex-1 m-0 bg-muted/20 overflow-y-auto p-8 flex justify-center">
            <Card className="w-full max-w-2xl h-fit p-8 shadow-sm">
              <h2 className="text-xl font-bold mb-6 italic text-primary">Form Settings</h2>
              <div className="space-y-6">
                <div className="grid gap-4">
                  <div className="grid gap-1.5">
                    <Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Form Name</Label>
                    <Input value={localName} onChange={(e) => handleNameChange(e.target.value)} />
                  </div>
                  <div className="grid gap-1.5">
                    <Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Public Slug</Label>
                    <div className="flex items-center gap-2">
                    ...
                    <p className="text-[10px] text-muted-foreground italic">Slug updates automatically as you change form name.</p>
                  </div>
                </div>
                ...
                <div className="grid gap-4">
                  <div className="grid gap-1.5">
                    <Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Campaign Connection</Label>
                    <Input value={form.campaignId || ""} readOnly className="bg-muted" placeholder="No campaign linked" />
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <h3 className="text-sm font-bold uppercase tracking-widest text-muted-foreground">General Configuration</h3>
                  <div className="flex items-center justify-between p-4 rounded-xl border bg-white shadow-sm">
                    <div className="space-y-0.5">
                      ...
                      <p className="text-xs text-muted-foreground">Notify counselors on new lead</p>
                    </div>
