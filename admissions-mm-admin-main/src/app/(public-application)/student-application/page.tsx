    ...
    <div className="w-full mb-10 px-1 sm:px-2">
      <div className="relative flex justify-between items-start">
        ...
        {steps.map((stepItem, idx) => {
          ...
          // Can jump to any step that has been reached previously (up to step 5 in form mode, or current step)
...
function MyApplicationForm({ isStudent }: { isStudent: boolean }) {
  ...
  const isStaff = user?.role === "org_admin" || user?.role === "superadmin" || user?.role === "application_manager";

  usePageHeader({
    title: isStaff ? "New Application" : "Admission Application",
    description: "Fill in the details below to submit a new admission application.",
    action: isStaff ? { label: "View Applications", href: "/organization/applications" } : undefined,
  });
  ...
  const finalizeSubmission = async () => {
    ...
    try {
      ...
      try {
        ...
        console.error("Failed to clear application draft after submission", e);
      }
  ...
  if (isPreview) {
    return (
      <div className={`w-full py-6 md:py-8 px-4 sm:px-6 lg:px-8 bg-white flex flex-col gap-6 ${manrope.className}`}>
        <Stepper currentStep={6} maxStepReached={5} onStepClick={handleStepClick} />
        ...
        <div className="mt-8 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-input pt-6">
          <Button
            type="button"
            variant="outline"
            onClick={handleResetForm}
            className="w-full sm:w-auto h-11 px-5 text-[14px] font-semibold text-rose-600 border-rose-200 hover:bg-rose-50 hover:text-rose-700 hover:border-rose-300 rounded-[8px] cursor-pointer transition-colors flex items-center justify-center gap-2 bg-white shadow-2xs"
          >
          ...
          {isStaff ? (
            <>
              <Button
                ...
                className="w-full sm:w-auto h-11 px-8 rounded-lg bg-[#2563EA] hover:bg-[#1d4ed8] text-white font-semibold text-sm shadow-sm cursor-pointer border-0"
              >
              ...
              <Button
                ...
                className="w-full sm:w-auto h-11 px-6 rounded-lg border-slate-300 text-slate-700 hover:bg-slate-50 font-semibold text-sm cursor-pointer bg-white"
              >
      ...
      <div className={`w-full pt-1 pb-8 px-4 sm:px-6 lg:px-8 bg-white ${manrope.className}`}>
        ...
        <div className="relative rounded-2xl border border-slate-200/80 bg-white shadow-xl shadow-slate-100 overflow-hidden">
          ...
          <div className="p-6 md:p-8 space-y-8">
            ...
            {/* Bottom Actions and Support note */}
            <div className="border-t border-slate-100 pt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
              <Button
                variant="outline"
                className="w-full sm:w-auto border-slate-300 h-10 px-5 text-xs font-semibold text-slate-700 rounded-lg hover:bg-slate-50 cursor-pointer"
                onClick={() => {
                  setShowPayment(false);
                  setIsPreview(true);
                }}
