...
Any time an API route is added, modified, or deleted, you MUST immediately record the change in `c:\WebMaddyProjects\admission-backend\API_DOCUMENTATION.md`.
...
Any time a database schema change is introduced (e.g., a new TypeORM entity is added, a column is appended, or an index is created), you MUST update `c:\WebMaddyProjects\admission-backend\Database.sql`.
